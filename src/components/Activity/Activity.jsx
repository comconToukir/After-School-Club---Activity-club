const Activity = ({ act: { name, picture, timeNeeded, description }, handleActivity }) => {
  return (
    <div className="font-noto text-gray-600 p-2 shadow-sm hover:shadow-md transition-shadow flex flex-col">
      <img src={picture} alt="" />
      <div className="p-2 mt-2 flex flex-col justify-between flex-grow">
        ...
        <button className="bg-blue-200 hover:bg-blue-300 transition-all w-full py-2 font-semibold shadow-sm" onClick={()=>handleActivity(timeNeeded)}>Add To Routine</button>
      </div>
    </div>
  );
};
